...
///
/// 이것이 나중에 실제 사용자 버그로 드러났다. 이 앱이 `max_tokens: 700`을
/// 보내고 있었고, 추론 모델에서 그 한도는 생각과 답변을 합쳐 세므로 본문이
/// 0자로 끝났다. **골든 데이터가 그 버그를 이미 담고 있었는데 그때는
/// 읽어내지 못했다.** 지금은 `docs/research/reasoning-model-budgets.md`에
/// 원인이 적혀 있고, `ChatRequest.maxTokens`의 기본값이 nil이다.
///
