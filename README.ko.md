...
- **근거 기반 상담** — 고민을 적으면 앱이 명리 축을 정하고 그 축의 근거만으로 풀이합니다. 축과 근거는 답을 받기 전에 보이고, 답변마다 그 답변이 실제로 쓴 근거가 붙습니다. 모르는 것은 지어내지 말고 모른다고 답하도록 지시받습니다.
- **채팅처럼 이어집니다** — <kbd>↩</kbd>로 보내고 <kbd>⇧</kbd><kbd>↩</kbd>로 줄을 바꿉니다. 지난 상담은 검색과 날짜 묶음으로 찾고, 답이 끝나면 그 근거로 더 물을 수 있는 것을 제안합니다. 제안은 입력창에 넣기만 하고 보내지 않습니다.
